class FantasyDraft::Generate < ApplicationService
  ...
  def call
    raise(ArgumentError.new("teams is not an array")) if !@teams.is_a?(Array)
    raise(ArgumentError.new("teams list is incomplete")) if @teams.size != 32
  ...
  def assign_players
    ...
    @roles.each_with_index do |role, round|
      teams.shuffle.each_with_index do |roster, index|
        ...
        roster[:players] << role.merge({offset: index})
      end
    end
