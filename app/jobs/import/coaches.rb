class Import::Coaches < Import::Job
  ...
  def get_values
    ...
    CSV.foreach(Rails.root.join("coaches.csv"), headers: true, header_converters: :symbol) do |row|
      next if row[:position] != "HeadCoach" || row[:level].to_i.zero? || row[:contractstatus] == "Retired" || row[:lastname] == "Madden"
      coach_id += 1
      ...
      overall_rating = [
        ...
        defense_rating.to_d * 0.37.to_d,
        specialteams_rating.to_d * 0.07.to_d
      ].sum.round(2)

      coaches << {
        id: coach_id,
        team_id: (row[:contractlength].to_i.zero? || row[:teamindex].to_i >= 32) ? nil : (row[:teamindex].to_i + 1),
